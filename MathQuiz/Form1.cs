...
namespace MathQuiz
{
    public partial class Form1 : Form
    {
        ...
        bool differenceCorrect = false;
        bool productCorrect = false;
        bool quotientCorrect = false;
        ...
        {
            if ((addend1 + addend2 == sum.Value) && !sumCorrect)
            {
                sumCorrect = true;
                SoundPlayer player = new SoundPlayer();
                player.Play();
            }
            if ((minuend - subtrahend == difference.Value) && !differenceCorrect)
            {
                differenceCorrect = true;
                SoundPlayer player = new SoundPlayer();
                player.Play();
            }
            if ((multiplicand * multiplier == product.Value) && !productCorrect)
            {
                productCorrect = true;
                SoundPlayer player = new SoundPlayer();
                player.Play();
            }
            if ((dividend / divisor == quotient.Value) && !quotientCorrect)
            {
